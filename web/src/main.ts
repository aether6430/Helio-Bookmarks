...
const render = (data: Bookmark[]) => {
  ...
  data.forEach((bookmark) => {
    ...
    card.className = "rounded-3xl border border-ink-200 bg-white p-6 shadow-soft";

    const tags = bookmark.tags.length
      ? `<div class=\"mt-3 flex flex-wrap gap-2\">${bookmark.tags
          .map(
            (tag) =>
              `<span class=\"rounded-full bg-ink-50 px-3 py-1 text-xs font-semibold text-ink-700\">${tag}</span>`
          )
          .join("")}</div>`
      : "";

    const image = bookmark.image
      ? `<div class=\"mt-4 overflow-hidden rounded-2xl border border-ink-200\"><img src=\"${bookmark.image}\" alt=\"${bookmark.title}\" class=\"h-40 w-full object-cover\" /></div>`
      : "";
    ...
    card.innerHTML = `
      <div class=\"flex flex-col gap-4\">
        <div class=\"flex items-start justify-between gap-4\">
          <div>
            <p class=\"text-xs uppercase tracking-[0.2em] text-moss-400\">${bookmark.siteName ?? "Saved"}</p>
            <h3 class=\"font-display text-xl font-semibold text-ink-900\">${bookmark.title}</h3>
            <a class=\"mt-2 inline-flex items-center gap-2 text-sm font-semibold text-moss-600 hover:text-moss-400\" href=\"${bookmark.url}\" target=\"_blank\" rel=\"noreferrer\">
              Visit link
            </a>
          </div>
        ...
        ${bookmark.description ? `<p class=\"text-sm text-ink-700\">${bookmark.description}</p>` : ""}
        ${bookmark.notes ? `<p class=\"rounded-2xl bg-ink-50 px-4 py-3 text-sm text-ink-700\">${bookmark.notes}</p>` : ""}
        ${tags}
        ${image}
        <div class=\"flex flex-wrap gap-2\">
          <button data-action=\"edit\" data-id=\"${bookmark.id}\" class=\"rounded-full border border-ink-200 px-4 py-2 text-xs font-semibold text-ink-700\">Edit</button>
          <button data-action=\"delete\" data-id=\"${bookmark.id}\" class=\"rounded-full border border-ink-200 px-4 py-2 text-xs font-semibold text-ink-700\">Delete</button>
        </div>
      </div>
    `;
